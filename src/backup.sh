...
mysqldump -h "$APP_DB_HOST" "-u $APP_DB_USER" "-p$APP_DB_PASSWORD" --add-drop-table --single-transaction --routines --triggers --compatible "$APP_DB_NAME" 2> "$APP_BACKUP_DIR/$APP_DB_NAME-$TIMESTAMP".log | gzip -c > "$APP_BACKUP_DIR/$APP_DB_NAME-$TIMESTAMP.tmp"
